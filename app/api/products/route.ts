...
export async function GET(req: NextRequest) {
  ...
  if (search) { wheres.push(`name ILIKE $${idx}`); vals.push(`%${search}%`); idx++; }
  ...
  try {
    const [countRes, dataRes] = await Promise.all([
      client.query(`SELECT COUNT(*) FROM products ${where}`, vals),
      client.query(`SELECT * FROM products ${where} ORDER BY id ASC LIMIT $${idx} OFFSET $${idx+1}`, [...vals, limit, offset])
    ]);
